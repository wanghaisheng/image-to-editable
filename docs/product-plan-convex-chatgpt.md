...
这里的 **RAMS** 指底层的推理、代理、多模型编排、记忆/状态与工具调度机制。结论是：**用户侧隐藏 RAMS 品牌与内部结构，技术侧保留 RAMS 能力但做平台级封装**。

### 7.1 用户侧：默认隐藏

面向普通创作者时，不建议把 RAMS 作为一级概念暴露。用户真正关心的是“我现在处于哪一步、需要确认什么、最终能导出什么”，而不是底层由哪个 Agent、哪段记忆或哪个调度器完成。因此用户界面应使用任务语言：

- 用“需求确认、效果图生成、可编辑重建、导出校验”替代“RAMS 流程”。
- 用“AI 正在分析页面结构 / 正在修复局部视觉差异”替代“某个 Agent 正在执行某个节点”。
- 在高级面板中展示可理解的过程证据，如页面清单、对象树、可编辑率、校验报告，而不是展示模型链路、prompt、内部记忆或调度细节。

例外场景是面向开发者、企业管理员或内部运营排障时，可以在“高级详情 / 调试日志 / 审计日志”中暴露 RAMS 相关诊断信息，但必须默认折叠，并避免让普通用户承担理解成本。

### 7.2 技术侧：不要消失，而是内聚封装

技术侧不应删除 RAMS，而应把它作为编排内核封装在 AI Orchestration Layer 之下。RAMS 的职责包括：

- 维护项目级状态：brief、outline、style、reference image、layout、export、qa、repair 的依赖关系。
- 决定任务路由：什么时候调用文本模型、视觉模型、图像模型、PPT/HTML builder 或 QA worker。
- 管理上下文与证据：用户确认项、批注、版本、失败对象、修复原因。
- 提供可追踪性：为每次导出保留输入、输出、模型版本、工具版本、校验结果和人工确认记录。

也就是说，RAMS 在技术侧应该是“可观测、可审计、可替换”的平台能力，而不是产品卖点本身。前端、Convex 表结构和 worker 应依赖稳定的项目状态与 job 接口，不直接依赖 RAMS 内部实现。

### 7.3 产品表达原则

| 层级 | 是否展示 RAMS | 推荐表达 | 不推荐表达 |
| --- | --- | --- | --- |
| 普通用户界面 | 否 | 项目阶段、下一步确认、生成进度、导出质量 | RAMS 节点、Agent 链路、内部记忆 |
| 高级用户/企业管理 | 弱展示 | 任务日志、版本记录、审计信息、成本与耗时 | 要求用户手动配置底层编排 |
| 内部运营/调试 | 是 | Trace、job graph、prompt/version、失败节点 | 只给不可解释的黑盒状态 |
| 技术架构 | 是，但封装 | Orchestration core、state machine、trace store | 让业务模块直接耦合内部 agent 实现 |

### 7.4 对 MVP 的影响

MVP 阶段建议先隐藏 RAMS，对外只呈现清晰的创作工作流；内部则保留最小可观测能力：`jobs.input`、`jobs.output`、`jobs.error`、`layouts.validationSummary`、资产版本和用户确认记录。等产品进入企业或开发者场景后，再把 RAMS 相关能力包装成高级调试、审计与自动化配置能力。
